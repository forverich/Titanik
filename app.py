...
with st.sidebar:
    ...
    page = st.radio(
        "Jump to section",
        ["🏠 Introduction", "📊 Demographics", "💀 Survival Analysis", "💰 Economic Factors", "📈 Correlations", "📝 Conclusion"],
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<h1 class='section-header'>📊 Passenger Demographics</h1>", unsafe_allow_html=True)
    st.markdown("<p class='section-sub'>Exploring the age and family structure of Titanic passengers</p>", unsafe_allow_html=True)

    # Quick stats row
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.markdown(f"""<div class='stat-card' style='--accent:#1F78B4'>
            <div class='stat-number'>{total}</div><div class='stat-label'>Filtered Passengers</div></div>""", unsafe_allow_html=True)
    with c2:
        st.markdown(f"""<div class='stat-number' style='font-family:Playfair Display,serif;font-size:1.8rem;color:#f5f0e8;'></div>""", unsafe_allow_html=True)
        st.metric("Avg Age", f"{avg_age:.1f} yrs")
    with c3:
        st.metric("Median Fare", f"£{df['Fare'].median():.1f}")
    with c4:
        st.metric("Avg Family Size", f"{df['FamilySize'].mean():.1f}")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<h1 class='section-header'>💀 Survival Analysis</h1>", unsafe_allow_html=True)
    st.markdown("<p class='section-sub'>How gender, class, port, age, and family size shaped survival odds</p>", unsafe_allow_html=True)

    c1, c2, c3 = st.columns(3)
    with c1:
        st.metric("Total (filtered)", total)
    with c2:
        st.metric("Survivors", int(survived))
    with c3:
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<h1 class='section-header'>💰 Economic Factors & Fare Analysis</h1>", unsafe_allow_html=True)
    st.markdown("<p class='section-sub'>How wealth translated directly into survival probability</p>", unsafe_allow_html=True)

    c1, c2, c3 = st.columns(3)
    c1.metric("Avg Fare", f"£{avg_fare:.1f}")
    c2.metric("Median Fare", f"£{df['Fare'].median():.1f}")
    c3.metric("Max Fare", f"£{df['Fare'].max():.1f}")

    st.markdown("<br>", unsafe_allow_html=True)

    # Figure 7: Fare boxplot
    ...
    if len(df_fig10) > 5:
        ...
        for surv_val in df_fig10['Survived'].unique():
            ...
            if len(subset) > 10:
                sns.regplot(data=subset, x='Age', y='Fare', scatter=False, lowess=True,
                            color=surv_colors_map[surv_val], line_kws={'linewidth': 2}, ax=ax10)
        ax10.set_yscale('log')
    ...
    if len(df_fig11) > 5:
        ...
        for ax, sv in zip(axes11, surv_vals):
            ...
            if len(subset) > 10:
                sns.regplot(data=subset, x='Age', y='Fare', scatter=False, lowess=True,
                            color=surv_colors_map.get(sv, '#888'), line_kws={'linewidth':2}, ax=ax)
            ax.set_yscale('log')
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<p class='section-sub'>Spearman rank correlations between continuous passenger variables</p>", unsafe_allow_html=True)
    ...
    # Additional: survival correlation bar
